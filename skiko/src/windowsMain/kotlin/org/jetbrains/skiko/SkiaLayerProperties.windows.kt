...
/** Rendering configuration for a Kotlin/Native Windows [SkiaLayer]. */
class SkiaLayerProperties(
    ...
    init {
        require(renderApi in SupportedWindowsRenderApis) {
            "Kotlin/Native Windows does not support $renderApi rendering"
        }
    }
...
/** Environment-backed rendering defaults for Kotlin/Native Windows. */
object SkikoProperties {
    ...
    internal fun fallbackRenderApiQueue(initialApi: GraphicsApi): List<GraphicsApi> {
        ...
        require(initialIndex >= 0) { "Kotlin/Native Windows does not support $initialApi rendering" }
        return all.drop(initialIndex)
    }

    internal fun parseRenderApi(value: String?): GraphicsApi =
        when (value?.uppercase()) {
            null, "", "DIRECT3D" -> GraphicsApi.DIRECT3D
            ...
            "SOFTWARE_COMPAT" -> GraphicsApi.SOFTWARE_COMPAT
                ...
                "Kotlin/Native Windows does not support $value rendering",
            )
        }
...
internal val SupportedWindowsRenderApis =
    setOf(
        GraphicsApi.DIRECT3D,
        GraphicsApi.OPENGL,
        GraphicsApi.SOFTWARE_FAST,
